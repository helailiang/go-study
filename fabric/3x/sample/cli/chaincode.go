...
func (c *Client) InstallCC(v string, orgPeer string) error {
	targetPeer := resmgmt.WithTargetEndpoints(orgPeer)
	...
	resps, err := c.rc.LifecycleInstallCC(req, targetPeer)
	if err != nil {
...
func (c *Client) InvokeCC(peers []string) (fab.TransactionID, error) {
	...
	args := packArgs([]string{"key1", "10-value"})
	req := channel.Request{
		ChaincodeID: c.CCID,
		Fcn:         "set",
		Args:        args,
